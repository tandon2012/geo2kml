...
class KmlMaker(object):
    ...
    def get_template(self,input_value,color_ramp=[2,5,10]):
        low = 2
        medium = 5
        high = 10
    ...
    def advanced_kml_output(self,title='Location Indicators',output_filename ='output_advanced.kml',color_ramp=[2,5,10],polygon_height=1000):
        f = open(output_filename,"w")
        header2 = header.replace('__title__',title)
        f.write(header2)
        for key,value in self.locations.items():
...
if __name__ == "__main__":
    ...
    kml.advanced_kml_output(output_filename=output_file, color_ramp=[2,5,10], polygon_height=1)
